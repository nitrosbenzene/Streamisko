...
function getSkTorrentHeaders(accept) {
  const headers = {
    "User-Agent": "Mozilla/5.0 Streamisko/0.9-series",
    Accept: accept,
...
function getTorBoxHeaders(extra = {}) {
  return {
    ...
    "User-Agent": "Streamisko/0.9-series",
    ...extra
  };
}
...
function parseSeriesVideoId(videoId) {
  const match = String(videoId || "").match(/^(tt\d+):(\d+):(\d+)$/i);
...
async function findSkTorrentSeriesResults(series) {
  if (!series || series.name === "Unknown series") return [];

  const primaryQuery = `${series.name} ${episodeCode(series.season, series.episode)}`;
  const primary = await searchSkTorrent(primaryQuery);
  if (primary.length) return primary;

  const fallbackQuery = `${series.name} ${series.season}x${pad2(series.episode)}`;
  return searchSkTorrent(fallbackQuery);
}
...
async function getTorBoxCacheMap(torrents) {
  ...
  for (let offset = 0; offset < hashes.length; offset += 50) {
    ...
    const cachedHashes = cachedHashesFromTorBoxPayload(resultResponse.payload);
...
function fileMatchesEpisode(file, season, episode) {
  const compact = torBoxFileName(file).toLowerCase().replace(/[^a-z0-9]/g, "");
  const seasonNumber = String(Number(season));
  const episodeNumber = String(Number(episode));
  const seasonPadded = pad2(season);
  const episodePadded = pad2(episode);

  return (
    compact.includes(`s${seasonPadded}e${episodePadded}`) ||
    compact.includes(`s${seasonNumber}e${episodeNumber}`) ||
    compact.includes(`${seasonNumber}x${episodePadded}`) ||
    compact.includes(`${seasonPadded}x${episodePadded}`)
  );
}

function chooseTorBoxEpisodeFile(files, season, episode) {
  if (!Array.isArray(files) || !files.length) return null;

  const videoExtension = /\.(mkv|mp4|m4v|avi|mov|webm|ts|m2ts|mpg|mpeg)$/i;
  const videoFiles = files.filter((file) => videoExtension.test(torBoxFileName(file)));
  const candidates = videoFiles.length ? videoFiles : files;
  const episodeMatches = candidates.filter((file) => fileMatchesEpisode(file, season, episode));
  const preferred = episodeMatches.length ? episodeMatches : candidates;
...
async function resolvePlayableTorBoxUrl(skTorrentId, season, episode) {
  ...
  const file = chooseTorBoxEpisodeFile(torrent.files, season, episode);
  if (!file || file.id == null) throw new Error("No playable episode file found in TorBox torrent");
...
function torrentToStream(torrent, index, baseUrl, series) {
  ...
  if (torrent.torBoxCached === true && torrent.infoHash) {
    const playUrl = new URL(`${baseUrl}/api/series`);
...
module.exports = async function handler(req, res) {
  ...
  if (route === "play") {
    ...
    if (!Number.isSafeInteger(season) || !Number.isSafeInteger(episode)) {
      return sendText(res, 400, "Invalid series episode");
    }

    try {
      const directUrl = await resolvePlayableTorBoxUrl(skTorrentId, season, episode);
      res.statusCode = 302;
  ...
  if (route !== "stream" || req.query.type !== "series") {
  ...
  const foundTorrents = await findSkTorrentSeriesResults(series);
  ...
  const episodeLabel = `${episodeCode(series.season, series.episode)}${series.episodeTitle ? ` • ${series.episodeTitle}` : ""}`;
  const helloStream = {
    name: "Streamiško",
    description: [
      "Hello Streamiško 👋",
      `${series.name} (${series.year}) • ${episodeLabel}`,
  ...
  return sendJson(res, 200, {
    streams: [
      helloStream,
      ...torrents.map((torrent, index) => torrentToStream(torrent, index, baseUrl, series))
    ]
  });
};
